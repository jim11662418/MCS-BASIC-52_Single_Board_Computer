...
; function for writing to 28CXXX EEPROM - Jim Loos 08/18/2024
PG7:              mov R4,A                      ; save the byte to be written
                  mov DPH,R2                    ; Load data pointer with EEPROM address
                  mov DPL,R0
                  movx @DPTR,A                  ; write the byte into the 28C256 EEPROM
                  movx A,@DPTR                  ; read the byte back from the 28C256
                  xrl A,R4                      ; compare the byte read back to the original byte in R0
                  jnz $-2                       ; loop back and re-read until the byte read matches the byte written
PG31:             ret
...
ZRO:              db 0
                  ...
                  ;**************************************************************
PGU:              MOV PSW,#00011000B          ; SELECT RB3
                  CJNE R6,#0,PG10
                  SJMP PG1

CERASE:           mov R7,#40H                 ; Erase 16K byte
                  mov R6,#00H
                  mov R2,#HI(ROMADR)-1       ; Startaddress EEPROM
                  mov R0,#LO(ROMADR)-1
...
ERA1:             lcall INC3210               ; Bump pointers
                  ...
                  ;R7: R6 gets # of bytes
                  ;*************************************************************
CCAL:             ACALL GETEND                ; GET THE LAST LOCATION
...
CCAL1:            RET
                  ...
                  ; Load the timer
                  ;*************************************************************
TIMER_LOAD:       ACALL CCAL1                 ; DELAY FOUR CLOCKS
TIMER_LOAD1:      CLR TR1                     ; STOP IT WHILE IT'S LOADED
                  MOV TH1,T_HH
                  MOV TL1,T_LL
                  ...
                  ;***************************************************************
CROM:             CLR CONB                    ; CAN'T CONTINUE IF MODE CHANGE
                  ACALL RO1                   ; DO IT
C_K:              LJMP CL3                    ; EXIT
RO1:              CALL DELTST
                  MOV R4,#1
...
X31DP:            XCH A,R3
                  ...
                  ;***************************************************************
                  ; Load the timer save location with the value the DPTR is pointing to.
                  ;****************************************************************
LD_T:             MOVX A,@DPTR
                  MOV T_HH,A
                  INC DPTR
...
ADDPTR1:          RET                         ; EXIT
                  ...
                  ;*************************************************************
LCLR:             ACALL ICLR                  ; CLEAR THE INTERRUPTS
...
S31DP:            MOV A,R3                    ; GET R3 INTO ACC
                  ...
                  ;***************************************************************
STRING:           LCALL TWO                   ; R3: R1 = NUMBER, R2: R0 = LEN
...
CCLR3:            JMP TB                      ; ALLOCATED MEMORY DOESN'T EXSIST
                  ...
                  ;**************************************************************
SCLR:             CALL DELTST                 ; TEST FOR A CR
                  JNC RCLEAR
                  ...
                  ;**************************************************************
ICLR:             JNB INTBIT,ICLR1            ; SEE IF BASIC HAS INTERRUPTS
...
INCH13:           CJNE A,#17H,CI_RET          ; CONTROL W
...
RROM:             SETB INBIT                  ; SO NO ERRORS
                  ...
                  ;WILL BE CLEARED
                  ;***************************************************************
CSTS:             JNB BI,CSTS1                ; BUBBLE STATUS
                  LJMP 2068H
...
SPEOP1:           ACALL SLET2                 ; EVALUATE AFTER =
                  ...
                  ;**************************************************************
IST_CAL:          ACALL I_PI                  ; BUMP TEXT, THEN EVALUATE
...
VARD:             CALL LDPTRI                 ; LOAD DPTR
                  ...
                  ;*************************************************************
CSY:              ACALL IST_CAL               ; CALCULATE IT
                  PUSH R3B0                   ; SAVE IT
                  PUSH R1B0
...
SGT21:            MOV C,ISAV
                  ...
                  ; Test for ZERO
                  ;*************************************************************
RTST:             ACALL EXPRB                 ; EVALUATE EXPRESSION
...
S_WU:             MOV R4,#DTYPE               ; DO EXPECTED
                  ...
                  ;***************************************************************
CNULL:            ACALL INTERR                ; GET AN INTEGER FOLLOWING NULL
...
EATC:             ACALL GCI                   ; GET THE CHARACTER
                  ...
                  ; ON Statement
                  ;***************************************************************
S_ON:             ACALL ONE                   ; GET THE EXPRESSION
...
SOT:              ACALL TWO                   ; GET THE NUMBERS
                  ...
                  ;***************************************************************
SCALL:            ACALL INTERR                ; CONVERT INTEGER
...
S_C_1:            ACALL AC1                   ; JUMP TO USER PROGRAM
                  ...
                  ;**************************************************************
THREE:            ACALL ONE                   ; GET THE FIRST INTEGER
...
TWO:              ACALL EXPRB
                  ...
                  ;*************************************************************
ONE:              ACALL EXPRB                 ; EVALUATE EXPERSSION
IFIXL:            CALL IFIX                   ; INTEGERS IN R3: R1
                  ...
                  ;*************************************************************
I_PI:             ACALL GCI1                  ; BUMP TEXT, THEN GET INTEGER
PAREN_INT:        ACALL P_E                   ; Get an integer in parens ( )
                  SJMP IFIXL
DP_B:             MOV DPH,BOFAH
                  MOV DPL,BOFAL
                  RET
...
T_DP:             MOV TXAH,DPH                ; TEXT POINTER GETS DPTR
                  ...
                  ; Check for a string
                  ;*************************************************************
S_C:              ACALL GC                    ; GET THE CHARACTER
...
PUSHAS:           CALL DEC_ASTKA
                  ...
                  ; Store at expression
                  ;***************************************************************
ST_A:             ACALL ONE                   ; GET THE EXPRESSION
                  SJMP POPAS                  ; SAVE IT

                  ;***************************************************************
                  ; Load at expression
                  ;***************************************************************
LD_A:             ACALL ONE                   ; GET THE EXPRESSION
                  ...
                  ;***************************************************************
CONST:            CALL GC                     ; FIRST SEE IF LITERAL
...
CN0t:             LCALL TWO_R2                 ; PUT IT ON THE STACK
...
DBTWO:            MOV DPTR,#NTWO
                  ...
                  ;*************************************************************
POLYC:            ACALL CSTAKA2               ; COPY THE STACK
...
CMPLK:            JMP FP_BASE2                ; DO THE COMPARE
                  ...
                  ;*************************************************************
DEC_ASTKA:        MOV A,#-FPSIZ
                  ADD A,ASTKA
                  CJNE A,#LO(TM_TOP)+6,DEC_ASTKA1
DEC_ASTKA1:       JC E4YY
                  MOV ASTKA,A
                  MOV R1,A
                  MOV R3,#ASTKAH
...
I21:              MOVX A,@R0                  ; GET DIGIT
                  ...
                  ;************************************************************
INC_ASTKA:        MOV A,#FPSIZ                ; NUMBER TO POP
                  SJMP SETREG1
...
LN_D:             RET
                  ...
                  ;*************************************************************
TEMP_COMP:        ACALL PUSH_T1               ; SAVE THE TEMP
...
AI3:              RET                         ; EXIT
                  ...
                  ; return sign in ACC
                  ;***************************************************************
AABS:             ACALL ANEG                  ; CHECK TO SEE IF + OR -
                  ...
                  ;***************************************************************
ASGN:             ACALL INC_ASTKA             ; POP STACK, GET EXPONENT
                  JZ P_Z                      ; EXIT IF ZERO
                  DEC R0                      ; BUMP TO SIGN
                  MOVX A,@R0                  ; GET THE SIGN
...
ALPAR:            RET
                  ...
                  ; Read the ROM
                  ;***************************************************************
ACBYTE:           ACALL IFIX                  ; GET EXPRESSION
                  ...
                  ;***************************************************************
ADBYTE:           ACALL IFIX                  ; GET THE EXPRESSION
                  ...
                  ;***************************************************************
AXBYTE:           ACALL IFIX                  ; GET THE EXPRESSION
AXBYTE1:          MOV P2,R3
                  MOVX A,@R1
                  AJMP TWO_R2
...
ALE:              ACALL CMPLK
                  ...
                  ;***************************************************************
ARND:             MOV DPTR,#RCELL             ; GET THE BINARY SEED
                  CALL L31DPI
                  MOV A,R1
                  CLR C
                  RRC A
...
ADIV:             LCALL FP_BASE4
                  ...
                  ; ON ERROR Statement
                  ;***************************************************************
SONERR:           LCALL INTERR                ; GET THE LINE NUMBER
                  SETB ON_ERR
...
PUSH_ONE:         MOV DPTR,#FPONE
                  AJMP PUSHC

POP_T1:           MOV R3,#HI(FPT1)
                  MOV R1,#LO(FPT1)
                  JMP POPAS

PUSH_T1:          MOV R0,#LO(FPT1)
PUSH_T11:         MOV R2,#HI(FPT1)
PUSH_T12:         LJMP PUSHAS
...
P_T2:             MOV R0,#LO(FPT2)
                  ...
                  ; SWAP TOS<>TOS-1
                  ;****************************************************************
SWAP_ASTKA:       ACALL SETREG                ; SET UP R2: R0 AND P2
...
TWOL:             ACALL IFIX
                  ...
                  ;*************************************************************
AGET:             MOV DPTR,#GTB               ; GET THE BREAK BYTE
                  MOVX A,@DPTR
                  JBC GTRD,TWO_R2
                  CLR A
...
AXTAL1:           ACALL CSTAKA2               ; COPY CRYSTAL VALUE TWICE
                  ...
                  JMP POPAS
AXTAL2:           ACALL AXTAL3
CBIAS:            MOV A,R1                    ; GET THE LOW COUNT
                  ...
                  ; Toggle the I/O port
                  ;**************************************************************
STONE:            CALL THREE                  ; GET THE NUMBERS
...
T_BUF:            MOV TXAH,#HI(IBUF)
                  ...
                  ;***************************************************************
CXFER:            CALL CCAL                   ; GET EVERYTHING SET UP
                  MOV R2,#HI(PSTART)
                  MOV R0,#LO(PSTART)
                  ...
                  ;***************************************************************
CRAM:             CLR CONB                    ; CAN'T CONTINUE IF MODE CHANGE
                  MOV BOFAH,#HI(PSTART)
                  MOV BOFAL,#LO(PSTART)
                  ...
                  ;***************************************************************
CMND1:            LCALL SPRINT1               ; WASTE AT AND HEX
...
TEMPD:            MOV TEMP5,DPH
                  ...
                  ; IDLE
                  ;**************************************************************
I_DL:             JB DIRF,E1XX                ; SYNTAX ERROR IN DIRECT INPUT
                  CLR DACK                    ; ACK IDLE
U_ID1:            ORL PCON,#01H
...
OTST:             LCALL GCI                   ; GET THE CHARACTER, CLEARS CARRY
...
FLOATING_SUB:     MOV P2,#ARG_STACK_PAGE
                  MOV R0,ARG_STACK
                  ...
                  ; floating point add
                  ;AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
FLOATING_ADD:     ACALL MDES1                 ; R7=TOS EXP, R6=TOS-1 EXP, R4=TOS SIGN
                  ...
                  ;**************************************************************
SWAP_AND_EXIT:    ACALL LOAD_POINTERS
...
SE1:              MOVX A,@R0                  ; SWAP THE ARGUMENTS
                  ...
                  DEC R1
                  DJNZ R7,SE1
POP_AND_EXIT:     MOV A,ARG_STACK             ; POP THE STACK
...
FSUB10:           AJMP UNDERFLOW_AND_EXIT
                  ...
                  ;***************************************************************
FLOATING_COMP:    ACALL MDES1                 ; SET UP THE REGISTERS
                  MOV A,ARG_STACK
...
EXPONENTS_EQUAL:  CJNE R5,#0,SIGNS_DIFFERENT
BOTH_PLUS:        MOV R7,#DIGIT               ; POINT AT MS DIGIT
                  DEC R0
                  DEC R0
                  DEC R0
                  DEC R1
...
CLOOP:            MOVX A,@R0
                  ...
                  ;MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
FLOATING_MUL:     ACALL MUL_DIV_EXP_AND_SIGN
...
NMARK_L:          MOV R6,A                    ; SAVE IT FOR LATER
                  ;Unpack and load R0
                  ACALL UNPACK_R0
...
UNDER_MD:         ajmp UNDERFLOW_AND_EXIT
...
FPC1:             MOV @R0,A
                  ...
                  ;**************************************************************
RIGHT:            MOV R4,#0                   ; IN CASE OF NO SHIFT
RIGHT1:           CLR C
...
RIGHT5:           MOV R7,A                    ; SAVE THE NEW SHIFT NUMBER
                  ...
                  ;***************************************************************
LEFT:             MOV R4,#00H                 ; CLEAR FOR SOME ENTRYS
LEFT1:            CLR C
...
MUL_NIBBLE:       ANL A,#0FH                  ; STRIP OFF MS NIBBLE
                  MOV R7,A
                  MOV R0,#FP_ACC8
                  MOV R1,#FP_NIB8
                  CLR A
...
MNL1:             INC R0
                  ...
                  ;***************************************************************
LOAD_POINTERS:    MOV P2,#ARG_STACK_PAGE
                  MOV R0,ARG_STACK
...
MDES1:            ACALL LOAD_POINTERS         ; LOAD REGISTERS
                  ...
                  ;***************************************************************
LOADR1_MANTISSA:  PUSH R0B0                   ; SAVE REGISTER 1
...
LOADR1:           MOVX A,@R1
                  ...
                  ;***************************************************************
HEXSCAN:          ACALL GET_DPTR_CHARACTER
                  PUSH DPH
...
HS1:              INC DPTR                    ; BUMP TO NEXT CHARACTER
                  SJMP HEXSC1                 ; LOOP
HEX_CHECK:        CLR ACC.5                   ; WASTE LOWER CASE
...
PUSHR2R0:         MOV R3,#HI(CONVERT)        ; CONVERSION LOCATION
                  MOV R1,#LO(CONVERT)
...
FINISH_UP:        MOV FP_EXP,R6               ; SAVE EXPONENT
...
PLUS_MINUS_TEST:  CJNE A,#0E3H,PMT11          ; SEE IF A PLUS, PLUS TOKEN FROM BASIC
                  SJMP PMT3
PMT11:            CJNE A,#'+',PMT12
                  SJMP PMT3
...
XOUT4:            CLR ACC.7
                  MOV R0,A
                  MOV R2,#0
                  MOV R1,#LO(CONVERT)         ; CONVERSION LOCATION
                  MOV R3,#HI(CONVERT)
                  ACALL CONVERT_BINARY_TO_ASCII_STRING
                  MOV R0,#LO(CONVERT)         ; NOW, OUTPUT EXPONENT
...
RSUB3:            djnz R5,RSUB2
                  ...
                  ;***************************************************************
HEXOUT:           ACALL SOUT                  ; OUTPUT A SPACE
...
;-------------------------------------------------------------------------------------------
; added BASIC COMMANDS and/or STATEMENTS

                  org 2002H
                  db 5AH                      ; tell BASIC that the expansion option is present

                  org 2048H
                  setb 45                     ; set the bit that indicates expansion option is present
                  ret

                  org 2070H
                  mov DPTR,#VECTOR_TABLE      ; set up DPTR to point to the expansion jump table
                  ret

                  org 2078H
                  mov DPTR,#USER_TABLE        ; set up DPTR to point to the expansion tokens table
                  ret

; expansion tokens table (maximum 16 tokens 10H-1FH)
USER_TABLE:       db 10H                      ; first token
                  db 'SETT0'                  ; user keyword
                  db 00H                      ; keyword terminator

                  db 11H                      ; second token
                  db 'CLRT0'                  ; second user keyword
                  db 00H                      ; keyword terminator

                  db 12H                      ; third token (16 max)
                  db 'CPLT0'                  ; third user token
                  db 00H                      ; keyword terminator
...
; expansion jump table
VECTOR_TABLE:     dw  sett0                  ; address of 'sett0' routine (token 10H)
                  dw  clrt0                  ; address of 'clrt0' routine (token 11H)
                  dw  cplt0                  ; address of 'cplt0' routine (token 12H)
                  dw  sett1                  ; address of 'sett1' routine (token 13H)
                  dw  clrt1                  ; address of 'clrt1' routine (token 14H)
                  dw  cplt1                  ; address of 'cplt1' routine (token 15H)
                  dw  monitor                ; address of 'monitor' routine (token 16H)

monitor:          lcall init_mon
                  ret

sett0:            setb T0                    ; set T0
                  ret

clrt0:            clr T0                     ; clear T0
                  ret

cplt0:            cpl T0                     ; complement T0
                  ret

sett1:            setb T1                    ; set T1
                  ret

clrt1:            clr T1                     ; clear T1
                  ret

cplt1:            cpl T1                     ; complement T1
                  ret

setp10:           setb P1.0                  ; call 20C4H
                  ret

clrp10:           clr P1.0                   ; call 20C7H
                  ret

cplp10:           cpl P1.0                   ; call 20CAH
                  ret

setp11:           setb P1.1                  ; call 20CDH
                  ret

clrp11:           clr P1.1                   ; call 20D0H
                  ret

cplp11:           cpl P1.1                   ; call 20D3H
                  ret

setp12:           setb P1.2                  ; call 20D6H
                  ret

clrp12:           clr P1.2                   ; call 20D9H
                  ret

cplp12:           cpl P1.3                   ; call 20DCH
                  ret

setp13:           setb P1.3                  ; call 20DFH
                  ret

clrp13:           clr P1.3                   ; call 20E2H
                  ret

cplp13:           cpl P1.3                   ; call 20E5H
                  ret

setp14:           setb P1.4                  ; call 20E8H
                  ret

clrp14:           clr P1.4                   ; call 20EBH
                  ret

cplp14:           cpl P1.4                   ; call 20EEH
                  ret

setp15:           setb P1.5                  ; call 20F1H
                  ret

clrp15:           clr P1.5                   ; call 20F4H
                  ret

cplp15:           cpl P1.5                   ; call 20F7H
                  ret

setp16:           setb P1.6                  ; call 20FAH
                  ret

clrp16:           clr P1.6                   ; call 20FDH
                  ret

cplp16:           cpl P1.6                   ; call 2100H
                  ret

setp17:           setb P1.7                  ; call 2103H
                  ret

clrp17:           clr P1.7                   ; call 2106H
                  ret

cplp17:           cpl P1.7                   ; call 2109H
                  ret

; =======================================================================================
; Serial monitor for the MCS-52 BASIC Single Board Computer
;
; NOTE: when using Tera Term to 'send' an Intel HEX file, make sure that the delay
; is set to at least 5 mSec/character and 500 mSec/line.
; =======================================================================================

; this macro is used to replace the 'movx @DPTR,A' instruction when writing to the 28C256 EEPROM.
; store the byte in ACC into 28C256 EEPROM addressed by DPTR. takes about 5.5 mSec/byte.
writeEEPROM    macro
               mov     R0,A                     ; save the byte to be written
               movx    @DPTR,A                  ; write the byte into the 28C256 EEPROM
               movx    A,@DPTR                  ; read the byte back from the 28C256
               xrl     A,R0                     ; compare the byte read back to the original byte in R0
               jnz     $-2                      ; loop back and re-read until the byte read matches the byte written
               mov     A,R0                     ; restore the original byte back to ACC
               endm

init_mon:      push    PSW
               lcall   puts
               db      CR,LF,'MCS-52 BASIC SBC Monitor Version 1.5',CR,LF
               db      "Assembled on ",DATE," at ",TIME,CR,LF,0               

get_cmd:       lcall   puts                     ; no match
               db      CR,LF
               db      " B - return to BASIC",CR,LF
               db      " C - Call subroutine",CR,LF
               db      " D - Display memory contents",CR,LF
               db      " E - Examine/modify memory",CR,LF
               db      " F - Fill memory with value",CR,LF
               db      " G - Go to address",CR,LF
               db      " L - Load intel hex file",CR,LF
               db      " M - Move external RAM",CR,LF,0

get_cmd1:      lcall   puts
               db      CR,LF,">>",0

               lcall   getch                    ; get input from the serial port, do not echo
               cje     A,#':',download          ; hex file download started
               anl     A,#01011111B             ; convert to upper case, strip parity
               cje     A,#'B',basic
               cje     A,#'C',call_sub
               cje     A,#'D',display_ram
               cje     A,#'E',examine_ram
               ...
               cje     A,#'L',download
               cje     A,#'M',move_extram
               ljmp    get_cmd                  ; go back to get another command

; -------------------------------------------------------------------
; return to the BASIC interpreter
; -------------------------------------------------------------------
basic:         lcall   puts
               db      CR,LF,LF,"Returning to BASIC...",CR,LF,0
               pop     PSW
               ret
...
; ------------------------------------------------------------------------
display_ram:   lcall   puts
               db      CR,LF,"External or Internal RAM (E or I)?",0
display_ram1:  lcall   getch
               anl     A,#01011111B             ; convert to upper case, strip parity
               cje     A,#'E',disp_extram
               cje     A,#'I',disp_iram
               ajmp    display_ram1
...
; ------------------------------------------------------------------------
; display one page of external memory in hex and ASCII.
; ------------------------------------------------------------------------
disp_extram:   lcall   puts
               db      CR,LF,"Address: ",0
               lcall   read_four                ; read address from serial port
               jnc     disp_extram1
               ljmp    get_cmd1

disp_extram1:  mov     A,DPL
               anl     A,#11110000B             ; start on 16-byte boundary
               mov     DPL,A
               mov     R2,#01H                  ; 1 page - 256 bytes
               mov     R3,#00H
               lcall   newline                  ; start on a new line
               push    DPH
               push    DPL
               lcall   puts
               db      CR,LF,"      00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F",CR,LF,0
               pop     DPL
               pop     DPH
disp_extram3:  mov     R4,#16                   ; set up byte counter
               ...
               mov     A,#':'
               lcall   putchar
               lcall   space                    ; and space to make it tidy
               push    DPH                      ; save DPTR for later
               push    DPL
disp_extram4:  movx    A,@DPTR                  ; output hex one byte at a time
               lcall   write_hex
               lcall   space                    ; a space to make it neat
               inc     DPTR
               djnz    R4,disp_extram4          ; if not done, loop

               lcall   space                    ; a space to make it neat
               pop     DPL                      ; restore the DPTR
               pop     DPH
               mov     R6,#16                   ; set up byte counter
disp_extram5:  movx    A,@DPTR                  ; output ASCII one byte at a time
               cjg     A,#127,disp_extram6      ; see if its >127
               cjl     A,#' ',disp_extram6      ; see if its <32
               sjmp    disp_extram7             ; its printable, so print it
disp_extram6:  mov     A,#'.'                   ; not printable, so make it '.'
disp_extram7:  lcall   putchar                  ; print it
               inc     DPTR
               djnz    R6,disp_extram5          ; if not done, loop

               lcall   newline                  ; go to next line
               ...
               jz      disp_extram8             ; if so, we're done
               jnc     disp_extram3             ; no carry, so not done yet

disp_extram8:  push    DPH
               push    DPL
               lcall   puts
               db      CR,LF,"Press SPACE for next page, ESCape to exit...",0
               pop     DPL
               pop     DPH
disp_extram9:  lcall   getch
               cje     A,#' ',disp_extram1
               cje     A,#ESC,disp_extram10
               sjmp    disp_extram9
disp_extram10: lcall   newline
               ljmp    get_cmd1


; ------------------------------------------------------------------------
; display internal RAM in hex and ASCII
; ------------------------------------------------------------------------
disp_iram:      lcall   puts
                db      CR,LF,"    00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F",CR,LF,0
                mov     R0,#00H
disp_iram3:     mov     A,R0
                lcall   write_hex               ; display the address
                mov     A,#':'
                lcall   putchar
                lcall   space
...
disp_iram4:     mov     A,@R0
                inc     R0
                lcall   write_hex               ; display the data
                lcall   space                   ; keep it pretty
                mov     A,R0
                anl     A,#00001111B
                cjne    A,#0,disp_iram4         ; 16 bytes at a time

                pop     ACC
                mov     R0,A
disp_iram5:     mov     A,@R0
                inc     R0
                cjg     A,#127,disp_iram6        ; see if its >127
                cjl     A,#' ',disp_iram6        ; see if its <32
                sjmp    disp_iram7               ; its printable, so print it
disp_iram6:     mov     A,#'.'                   ; not printable, so make it '.'
disp_iram7:     lcall   putchar                  ; print it
                mov     A,R0
                anl     A,#00001111B
                cjne    A,#0,disp_iram5         ; 16 bytes at a time

                lcall   newline                 ; bump down to next line
                cjne    R0,#0,disp_iram3        ; loop 'till R3 overflows
                ljmp    get_cmd1                ; and return to command mode

; -------------------------------------------------------------------
; Examine/modify RAM
; -------------------------------------------------------------------
examine_ram:   lcall   puts
               db      CR,LF,"External or Internal RAM (E or I)?",0
examine_ram1:  lcall   getch
               anl     A,#01011111B             ; convert to upper case, strip parity
               cje     A,#'E',mod_extram
               cje     A,#'I',mod_iram
               ajmp    examine_ram1
...
; -------------------------------------------------------------------
; Examine/modify internal RAM
; ENTER or ESCape key exits. SPACE increments to the next address.
; -------------------------------------------------------------------
mod_iram:      lcall   puts
               db      CR,LF,"Address: ",0
               lcall   read_two                 ; get the internal RAM address to read/modify
               jnc     mod_iram1                ; continue if not ESCape key or enter key
               ljmp    get_cmd1                 ; else, exit

mod_iram1:     mov     R0,A
mod_iram2:     lcall   newline
               mov     A,R0
               lcall   write_hex                ; print the internal RAM address
               lcall   puts
               db      "   --> ",0
               mov     A,@R0
               lcall   write_hex                ; show the byte retrieved from internal RAM
               lcall   puts
               db      "   New value: ",0
               lcall   read_two                 ; get the new value to store in internal RAM
               jnc     mod_iram4                ; continue if not ESCape key or enter key
               cje     A,#' ',mod_iram5         ; space increments to next address
               lcall   newline
               ljmp    get_cmd1                 ; anything else, exits

mod_iram4:     mov     @R0,A
mod_iram5:     inc     R0                       ; next internal RAM address
               ajmp    mod_iram2

; ----------------------------------------------------------------------
; Examine/Modify external RAM
; ENTER or ESCape key exits. SPACE increments to the next address.
; ----------------------------------------------------------------------
mod_extram:    lcall   puts
               db      CR,LF,"Address: ",0
               lcall   read_four
               jnc     mod_extram1
               ljmp    get_cmd1

mod_extram1:   lcall   newline                  ; go to a new line
               ...
               pop     DPL
               pop     DPH
               lcall   read_two                 ; get the new value
               jnc     mod_extram5
               cje     A,#' ',mod_extram6       ; space increments to next address
               lcall   newline
               ljmp    get_cmd1                 ; anything else, exits

mod_extram5:   writeEEPROM                      ; write to external 28C256 EEPROM
mod_extram6:   inc     DPTR                     ; point to next location
               sjmp    mod_extram1              ; and continue
...
; ------------------------------------------------------------------------
download:      mov     R7,#0                    ; clear error count
               cje     A,#':',download3         ; start of Intel hex record (':') received at the '>>' prompt
               lcall   puts                     ; prompt for start of xfer
               db      CR,LF,'Waiting for hex download...',CR,LF,0

download1:     lcall   getch                    ; get the first character
               cjne    A,#esc,download2
               lcall   newline
               ljmp    get_cmd1                 ; exit if escape key

download2:     cjne    A,#':',download1         ; go back if not the  start of record character
               lcall   putchar                  ; else, echo the character
download3:     lcall   read_two                 ; get byte count
               jz      download6                ; last record has byte count of zero
...
download4:     lcall   read_two                 ; get the data byte
               writeEEPROM                      ; write to external 28C256 EEPROM
               add     A,R6                     ; add to checksum
               mov     R6,A
               inc     DPTR                     ; move DPTR to next byte address
               djnz    R5,download4             ; loop until finished
...
; values, including the record's checksum, and verifying that the LSB of the sum is zero.
               ...
               add     A,R6                     ; add the decoded checksum to the record's checksum
               jz      download5                ; zero means the checksums agree
               inc     R7                       ; else, increment the checksum error counter
download5:     lcall   getche                   ; get the carriage return at the end of the line
               ajmp    download2                ; go back for next line

download6:     lcall   read_two                 ; get the last address
               lcall   read_two
               lcall   read_two                 ; get the last record type
               lcall   read_two                 ; get the last checksum
               lcall   getche                   ; get the last cr

               lcall   newline
               mov     A,R7                     ; get error count
               lcall   pint8u                   ; print the error count in decimal
               lcall   puts
               db      ' checksum errors.',CR,LF,0
               ljmp    get_cmd1

; -------------------------------------------------------------------
; jumps to the address entered from the serial port.
; -------------------------------------------------------------------
go:            lcall   puts
               db      CR,LF,'Address: ',0
               lcall   read_four
               jnc     go1                      ; continue below if not ESCAPE, ENTER or SPACE
               ljmp    get_cmd1                 ; else, return to monitor

go1:           lcall   newline
               push    DPL                      ; push new address entered above onto the stack
               push    DPH
               ret                              ; pop the new address off the stack into PC

; ---------------------------------------------------------------------
; call a user subroutine, return to monitor
; ---------------------------------------------------------------------
call_sub:      lcall   puts
               db      CR,LF,'Address: ',0
               lcall   read_four
               jnc     call_sub2                ; continue below if not ESCAPE, ENTER or SPACE
call_sub1:     ljmp    get_cmd1                 ; else, return to monitor

call_sub2:     mov     R0,DPL                   ; save the address of the subroutine to call in R0 and R1
               mov     R1,DPH
               lcall   newline
               mov     DPTR,#call_sub1          ; get the address of 'call_sub1' above
               push    DPL                      ; push the address of 'call_sub1' onto the stack
               push    DPH
               push    R0B0                     ; push the address of the user subroutine to be called onto the stack
               push    R1B0
               ret                              ; 'ret' pops the address of the user subroutine off the stack into PC

; -------------------------------------------------------------
; fill external RAM with a byte.  DPTR holds the target address.
; R2, R3 hold the number of locations to fill.
; NOTE: be patient! it takes about 2 minutes, 20 seconds to fil
; l the entire 32,768 bytes of the 28C256.
; -------------------------------------------------------------
fill_extram:    lcall   puts
                db      CR,LF,'Address: ',0
...
fill_extram0:   push    DPH
                ...
                pop     DPL
                pop     DPH
                jc      fill_extram2                   ; exit if escape
                inc     R2                      ; fix djnz wierdity
                inc     R3                      ; fix djnz wierdity
                push    DPH
                push    DPL
                lcall   puts
                db      '   Value: ',0
                pop     DPL
                pop     DPH
                lcall   read_two                ; byte to fill_extram with
                jc      fill_extram2

fill_extram1:   writeEEPROM                     ; write to external 28C256 EEPROM
                inc     DPTR                    ; point to the next location
                djnz    R3,fill_extram1         ; loop if not done
                djnz    R2,fill_extram1         ; loop if not done
fill_extram2:   ljmp    get_cmd1


; -----------------------------------------------------------
; move a block of external RAM.  R2, R3 hold the number of bytes to move.
; R4, R5 hold the source address.  R6, R7 hold the destination address.
; ------------------------------------------------------------
move_extram:    lcall   puts
                db      CR,LF,'Source: ',0
...
move_extram1:   mov     DPH,R4                  ; high byte of source address
                ...
                mov     DPH,R6                  ; high byte of dest address
                mov     DPL,R7                  ; low byte of dest address
                writeEEPROM                     ; write to external 28C256 EEPROM
                inc     DPTR                    ; point to the next location
                mov     R6,DPH                  ; save the high byte
                mov     R7,DPL                  ; save the low byte
                djnz    R3,move_extram1         ; loop if not done
                djnz    R2,move_extram1         ; loop if not done
move_extram2:   ljmp    get_cmd1

; ------------------------------------------------------------------------
; reads two hex characters from the serial port and converts them into a
; byte returned in ACC.  returns with carry flag set if space, return or escape key is pressed.
; ------------------------------------------------------------------------
...
read_two1:      cje     A,#esc,read_two6        ; jump if escape key
                ...
                cje     A,#' ',read_two6        ; jump if space bar
...
; space, return or escape key is pressed.
; ------------------------------------------------------------------------
read_four:      mov     DPL,#0
                mov     DPH,#0
                lcall   get_hex                 ; get first digit
                jc      read_four4              ; exit if enter, escape, or space
                lcall   read_four5              ; else read_four5 digit into position
...
read_four5:     lcall   asc2hex                 ; convert the ASCII character to a hex nibble
...
; -------------------------------------------------------------------------
; writes byte in ACC to the serial port as two ASCII characters 00-FF.
; -------------------------------------------------------------------------
write_hex:      push    B
                mov     B,R6
                push    B
...
; ------------------------------------------------------------------------
; converts an ASCII character in ACC to its hex equivalent.
...
; ------------------------------------------------------------------------
; converts the lower nibble in ACC to an ASCII character
; ------------------------------------------------------------------------
hex2asc:        push    B
                anl     A,#00001111B
                clr     CY
...
; Leading zeros are supressed. ACC is not changed, but Carry and PSW.5 (F0) are changed.
; pint8  and pint9u utility routines courtesy of pjrc.com...
; ------------------------------------------------------------------------
pint8u:         push    B
                push    ACC
                sjmp    pint8b
...
; Twos complement format is assumed: 0xFF = '-128', 0x7F = '127'.
; ------------------------------------------------------------------------
pint8:          push    B
                push    ACC
                jnb     ACC.7,pint8b
...
puts1:          clr     A                       ; (zero offset)
                ...
                lcall   putchar                 ; send it out
                inc     DPTR                    ; bump pointer
                clr     A
                movc    A,@a+DPTR               ; get next character
                cjne    A,#0,puts1              ; loop until zero read
                mov     A,#1
...
; -----------------------------------------------------------------
; outputs a carriage return and line feed out the serial port
; -----------------------------------------------------------------
newline:        push    ACC
                mov     A,#CR
                lcall   putchar
...
; ------------------------------------------------------------------
; gets a character from the serial port. echos the character if it is 0-F
; ------------------------------------------------------------------
getdigit:       lcall   getch
                anl     A,#7FH                  ; strip off parity bit
                cjl     A,#'0',getdigit6        ; return without echo if less than '0'
                cjg     A,#'z',getdigit6        ; return without echo if above 'z'
                cjl     A,#'a',getdigit5
                subb    A,#20H                  ; convert to upper case
getdigit5:      lcall   putchar                 ; echo all other characters
getdigit6:      ret


; ------------------------------------------------------------------
; returns a hex character in ACC from the serial port. echos the character
; back to the serial port. returns with carry set if ENTER, ESCAPE, or SPACE
; ------------------------------------------------------------------
get_hex:        lcall   getch
...
get_hex2:       lcall   putchar                 ; echo the character
...
; ------------------------------------------------------------------------
; sends the character in ACC to the serial port.
; ------------------------------------------------------------------------
putchar:        jnb TI,$                        ; wait for transmit ready
                clr TI
                mov SBUF,A
                ret

; ------------------------------------------------------------------
; waits for a character from the serial port. echos the character back
; to the serial port. returns the character in ACC.
; ------------------------------------------------------------------
getche:         lcall   getch
                lcall   putchar                 ; echo it
                ret

; ------------------------------------------------------------------
; waits for a character from the serial port. returns the character in ACC.
